...
def main():
    ...
    logger.info("\nrunning GA")
    best, best_vehicle, best_offsets = joint_optimize(
        track,
        pop_size=40,
        n_gens=30,
        n_control=20,
    ...
    traj = tb.build_trajectory(track, track.get_centerline(), best_offsets)
    print("VEHICLE FOR ES: ", best_vehicle)
    print("TRAJ FOR ES: ", traj)
    best_params, es_hist = es_optimize_throttle(traj, track, best_vehicle,
                                            pop_size=20, n_gens=50, Np=30, verbose=True)
    
    print("ES best params: ", best_params)
    print("ES best median multiplier (approx):", float(np.median(best_params)))
    print("FINISHED ES")

    best_vehicle.max_power *= float(np.median(best_params))
    racing_line = traj
    ...
    sim = LapSimulator(best_vehicle, racing_line)
